...
public class FirstUnique {
    public static int solution(int[] A) {
        ...
        for(int pos=0; pos<A.length ; pos++){
            ...
            if(!uniques.contains(value)){
                if(!numberPosition.containsKey(value)){
                    numberPosition.put(value, pos);
                }
                uniques.add(value);
            }else{
                numberPosition.remove(value);
            }
        }
        if(numberPosition.isEmpty()){
            return -1;
        }else{
            int first = A.length;
            int key = -1;
    ...
    public static int chatGPT(int[] A) {
        ...
        // Step 2: Iterate through the array to find the first unique number
        for (int num : A) {
            if (countMap.get(num) == 1) { // this is a unique number
                return num; // return the first unique number found
            }
        }

        return -1; // if no unique number is found, return -1
    }
